          ...
          <div className="eg-panel flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden" role="dialog" aria-modal="true" aria-labelledby="campaign-detail-title" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="eg-safe-scroll flex-1 overflow-y-auto p-5 md:p-7">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="eg-card p-4"><CalendarClock className="h-4 w-4 text-cyan-100/60" /><span className="mt-3 block text-[9px] uppercase tracking-wider text-zinc-600">Deadline</span><strong className="mt-1 block text-xs text-white">{formatDate(selectedCampaign.deadline, localeTag, t('common.notSet'))}</strong></div>
                <div className="eg-card p-4"><CircleDollarSign className="h-4 w-4 text-cyan-100/60" /><span className="mt-3 block text-[9px] uppercase tracking-wider text-zinc-600">Ngân sách</span><strong className="mt-1 block text-xs text-white">{formatBudget(selectedCampaign.budget, selectedCampaign.currency, localeTag)}</strong></div>
              ...
              <section className="mt-6"><div className="flex items-end justify-between gap-3"><div><div className="eg-kicker">Production outputs</div><h3 className="mt-1 text-base font-semibold text-white">Đầu ra phải bàn giao</h3></div><span className="eg-chip"><Film className="h-3 w-3" /> {getCampaignDeliverableCount(selectedCampaign)} video</span></div><div className="mt-4 space-y-3">{selectedCampaign.deliverables.map((deliverable) => (
                <article key={deliverable.id} className="eg-card flex flex-col gap-4 p-4 lg:flex-row lg:items-center">
                  ...
                  <div className="min-w-0 flex-1"><h4 className="truncate text-sm font-semibold text-white">{deliverable.title}</h4><p className="mt-1 text-[10px] text-zinc-600">{platformLabels[deliverable.platform]} · {deliverable.aspectRatio} · {deliverable.duration}s · {deliverable.quantity} phiên bản</p></div>
