...
import { Check, Copy, Loader2, LogOut, MailPlus, ShieldCheck, UserRound, UsersRound, X } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import { useLocale } from '../../contexts/LocaleContext';
import { EgoricRole, TeamState, ROLE_LABELS, createInvite, loadTeam, updateTeamUser } from '../../services/authService';
        ...
        <div className="space-y-6 p-5 md:p-7">
          ...
          {canManage && (
            <section className="rounded-2xl border border-white/[.08] bg-white/[.025] p-5">
              ...
              {inviteUrl && <div className="mt-4 rounded-xl border border-emerald-300/15 bg-emerald-300/[.05] p-3"><div className="mb-2 text-[10px] font-semibold text-emerald-100">{vi ? 'Link chỉ hiển thị ở lần tạo này' : 'This link is shown only once'}</div><div className="flex gap-2"><input readOnly value={inviteUrl} className="eg-auth-input min-w-0 flex-1 font-mono text-[10px]" /><button type="button" onClick={() => void copyInvite()} className="eg-button-secondary flex min-h-11 shrink-0 items-center gap-2 px-3 text-xs">{copied ? <Check className="h-4 w-4 text-emerald-300" /> : <Copy className="h-4 w-4" />}{copied ? (vi ? 'Đã chép' : 'Copied') : (vi ? 'Sao chép' : 'Copy')}</button></div></div>}
            </section>
          )}
          ...
          {canSeeTeam && (
            <section className="rounded-2xl border border-white/[.08] bg-white/[.02] p-5">
              ...
              {!!team?.invites.length && <div className="mt-5 border-t border-white/[.06] pt-4"><div className="mb-3 text-[9px] font-semibold uppercase tracking-[.16em] text-zinc-600">{vi ? 'Đang chờ kích hoạt' : 'Pending invitations'}</div><div className="space-y-2">{team.invites.map((item) => <div key={`${item.email}-${item.expiresAt}`} className="flex items-center justify-between gap-3 rounded-lg bg-white/[.02] px-3 py-2 text-[10px]"><span className="truncate text-zinc-400">{item.email}</span><span className="shrink-0 text-zinc-600">{ROLE_LABELS[item.role][locale]}</span></div>)}</div></div>}
            </section>
          )}
        </div>
      </aside>
