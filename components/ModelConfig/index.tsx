...
const ModelConfigModal: React.FC<ModelConfigModalProps> = ({ isOpen, onClose }) => {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode }[] = [
    { id: 'global', label: t('model.tabGlobal'), icon: <Key className="w-4 h-4" /> },
    { id: 'chat', label: t('model.tabChat'), icon: <MessageSquare className="w-4 h-4" /> },
    { id: 'image', label: t('model.tabImage'), icon: <Image className="w-4 h-4" /> },
    { id: 'video', label: t('model.tabVideo'), icon: <Video className="w-4 h-4" /> },
  ];
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">{t('model.modalTitle')}</h2>
          ...
          <button
            ...
            aria-label={t('model.closeConfig')}
          >
            <X className="w-4 h-4" />
          </button>
        </div>
