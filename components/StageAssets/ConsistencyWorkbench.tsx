      ...
      <div className="flex flex-col xl:flex-row xl:items-start gap-5">
        <div className="xl:w-72 shrink-0">
          <div className="flex items-center gap-2 text-cyan-200">
            ...
            <h3 className="text-sm font-bold uppercase tracking-widest">Khóa nhất quán</h3>
          </div>
          <p className="mt-2 text-xs leading-relaxed text-zinc-500">
            Chọn đúng tài sản bắt buộc xuất hiện. Reference Pack sẽ tự đưa chúng vào mọi shot và vẫn có prompt dự phòng khi thiếu ảnh.
          </p>
          <div className="grid grid-cols-3 gap-2 mt-4">
            <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
              <p className="text-lg font-bold text-white">{readyCharacters}/{characterReports.length}</p>
              <p className="text-[8px] uppercase tracking-wider text-zinc-600">Nhân vật sẵn sàng</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
              <p className="text-lg font-bold text-white">{lockedScenes}</p>
              <p className="text-[8px] uppercase tracking-wider text-zinc-600">Bối cảnh khóa</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
              <p className={`text-lg font-bold ${riskCount ? 'text-amber-300' : 'text-emerald-300'}`}>{riskCount}</p>
              <p className="text-[8px] uppercase tracking-wider text-zinc-600">Cảnh báo</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Tài sản Brand Kit bắt buộc</p>
            </div>
            <span className="text-[9px] text-zinc-600">Bấm để khóa / bỏ khóa</span>
          </div>
          ...
          {assets.length ? (
              ...
              {assets.map((asset) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={locked}
                    aria-label={`${locked ? 'Bỏ khóa' : 'Khóa'} ${ASSET_LABELS[asset.type]} ${asset.name}`}
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[8px] uppercase tracking-wider text-cyan-200/55">{ASSET_LABELS[asset.type]}</p>
                      <p className="text-[9px] text-zinc-600 mt-1 flex items-center gap-1">
                        <LockKeyhole className="w-2.5 h-2.5" />
                        {locked ? 'Đang khóa' : 'Không dùng'}
                      </p>
                    </div>
                  </button>
                );
            ...
            <div className="rounded-2xl border border-dashed border-amber-300/20 bg-amber-300/[0.025] px-4 py-3 flex gap-3">
              ...
              <p className="text-[10px] leading-relaxed text-amber-100/55">
                Brand Kit chưa có logo hoặc ảnh sản phẩm. Nhân vật và bối cảnh vẫn tạo được bằng prompt; hãy thêm ảnh chuẩn để khóa bao bì và nhận diện thương hiệu.
              </p>
            </div>
          )}
        </div>
